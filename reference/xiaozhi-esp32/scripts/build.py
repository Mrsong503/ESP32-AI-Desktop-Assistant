...
# 注意：不使用 .resolve()，保留 junction 路径（如 C:/xiaozhi），
# 避免 ESP-IDF kconfig 工具无法处理 Unicode 路径（如 Eliya's mind 中的 U+2018）
...
def _run_idf(*args: str, preview: bool = False) -> None:
    # Windows 下 idf.py 是 Python 脚本，需用 Python 解释器执行
    # 优先使用 IDF_PATH 环境变量定位 idf.py
    idf_path = os.environ.get("IDF_PATH", "")
    idf_py = os.path.join(idf_path, "tools", "idf.py") if idf_path else "idf.py"
    if sys.platform == "win32" and os.path.exists(idf_py):
